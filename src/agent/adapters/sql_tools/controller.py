...
from src.agent.adapters.sql_tools.aggregation_agent import AggregationAgent
from src.agent.adapters.sql_tools.filter_agent import FilterAgent
from src.agent.adapters.sql_tools.grounding_agent import GroundingAgent
from src.agent.adapters.sql_tools.join_inference_agent import JoinInferenceAgent
from src.agent.adapters.sql_tools.models import SQLGenerationResponse
from src.agent.adapters.sql_tools.sql_construction_agent import SQLConstructionAgent
from src.agent.adapters.sql_tools.validation_agent import ValidationAgent
from src.agent.config import get_database_config, get_llm_config
...
class SQLController:
    ...
    def process_question(self, question: str) -> SQLGenerationResponse:
        ...
        try:
            ...
            filter_result = self.filter_agent.process(
                question, grounding_result.column_mappings
            )

            # Step 3: Determine necessary joins
            join_result = self.join_inference_agent.process(
                question, grounding_result.table_mappings
            )

            # Step 4: Detect aggregation requirements
            aggregation_result = self.aggregation_agent.process(
                question, grounding_result.column_mappings
            )
            ...
            validation_result = self.validation_agent.process(
                question, construction_result.sql_query
            )
            ...
            if validation_result.is_valid:
                ...
                final_sql = (
                    construction_result.sql_query
                )  # Use original even if invalid
